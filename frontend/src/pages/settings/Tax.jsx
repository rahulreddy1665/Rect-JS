...
function HSNTax() {
  ...
  const form = useForm({
    ...
    validate: {
      label: (value) => (value.length < 1 ? "HSN Tax name is required" : null),
    },
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        ...
        const response = await handleGetHSNTax();
  ...
  const openConfirmModal = (e) => {
    ...
    modals.openConfirmModal({
      title: "Do you want to delete this tax value",
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteHSNTax(e);
    ...
    if (response.status === 200) {
      notificationHelper({
        color: "green",
        title: "Success",
        message: "HSN Tax deleted successfully",
      });
  ...
  const handleEdit = (e) => {
    ...
    formEdit.setFieldValue("tax", Number(datas.tax));
    setVariables({ ...variables, deleteIndex: e, openEdit: true });
  };

  // Edit tax data compoennt
  const EditHSNTax = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleEditHSNTax(e);
    // Check for respose data for actions
    if (response.status === 200) {
      notificationHelper({
        color: "green",
        title: "Success",
        message: "HSN Tax updated successfully",
      });
      setVariables({
        ...variables,
        submitLoading: false,
  ...
  // Add new tax data
  const AddHSNTax = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleAddHSNTax(e);
    // Check for response for actions
    if (response.status === 200) {
      notificationHelper({
        color: "green",
        title: "Success",
        message: "HSN Tax added successfully",
      });
      form.reset();
      setVariables({
        ...variables,
  ...
  // Upload bulk json data tax
  const BulkDataUpload = async () => {
    setVariables({ ...variables, submitLoading: true });

    const response = await handleBulkHSNTax(bulkData);
    // Check for response for actions
    if (response.status === 200) {
      interval.stop();
      notificationHelper({
        color: "green",
        title: "Success",
        message: "Bulk tax added successfully",
      });
      form.reset();
      setVariables({
        ...variables,
  ...
  const print = () => {
    const doc = new jsPDF();
    var body = [...variables.data.map((el) => [el.label, el.tax])];
    doc.autoTable({
  ...
  return (
    <div>
      ...
      <Skeleton radius="md" visible={variables.skeletonLoading}>
        <BreadCrumb Text="HSN Tax" Title="Settings" titleLink="/manages" />
      </Skeleton>
      ...
      >
        ...
        <div className="zc-p-3">
          {/* HSNTax adding form name */}
          <form onSubmit={form.onSubmit((values) => AddHSNTax(values))}>
            <TextInput
              variant="filled"
              value={form.values.label}
              label="HSN Name"
              placeholder="HSN Name"
      ...
      >
        ...
        <div className="zc-p-3">
          {/* HSNTax adding form name */}
          <form onSubmit={formEdit.onSubmit((values) => EditHSNTax(values))}>
            <TextInput
              variant="filled"
      ...
      {/* HSNTax Bulk drawer end */}
    </div>
  );
}
export default HSNTax;
