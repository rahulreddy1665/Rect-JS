...
function Address() {
  ...
  const form = useForm({
    ...
    validate: {
      label: (value) => (value.length < 1 ? "Address name is required" : null),
    },
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        //   For get all the address data list
        const response = await handleGetAddress();
  ...
  const openConfirmModal = (e) => {
    ...
    modals.openConfirmModal({
      title: "Do you want to delete this address value",
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteAddress(e);
  ...
  const handleEdit = (e) => {
    ...
    formEdit.setFieldValue("door", datas.door);
  ...
  // Edit address data compoennt
  const EditAddress = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleEditAddress(e);
  ...
  const AddAddress = async (e) => {
    setVariables({ ...variables, submitLoading: true });
    const response = await handleAddAddress(e);
  ...
  // Upload bulk json data address
  const BulkDataUpload = async () => {
    setVariables({ ...variables, submitLoading: true });

    const response = await handleBulkAddress(bulkData);
    // Check for response for actions
    if (response.status === 200) {
      interval.stop();
      notificationHelper({
        color: "green",
        title: "Success",
        message: "Bulk address added successfully",
      });
      form.reset();
      setVariables({
        ...variables,
  ...
  return (
    <div>
      ...
      <Skeleton radius="md" visible={variables.skeletonLoading}>
        <BreadCrumb Text="Address" Title="Settings" titleLink="/manages" />
      </Skeleton>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div>
                <Group spacing="xs">
                  ...
                  <CSVLink
                    data={variables.data}
                    headers={[{ label: " Address Name", key: "label" }]}
                  ...
                  {/* Drawer open for adding new address data */}
                  <Button
                    variant="outline"
                    color="zevcore"
                    size="xs"
      ...
      >
        ...
        <div className="zc-p-3">
          ...
          {/* Address adding form name */}
          <form onSubmit={form.onSubmit((values) => AddAddress(values))}>
            <TextInput
              variant="filled"
      ...
      >
        ...
        <div className="zc-p-3">
          {/* Address adding form name */}
          <form onSubmit={formEdit.onSubmit((values) => EditAddress(values))}>
            <TextInput
              variant="filled"
            ...
            <TextInput
              variant="filled"
              value={formEdit.values.door}
              label="Door No."
              placeholder="Door No."
              {...formEdit.getInputProps("door")}
            />
            <TextInput
              variant="filled"
      ...
      >
        ...
        <div className="zc-p-3">
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <div
              onClick={() => onDownload({ data: "address" })}
              style={{
