...
import {
  ...
  handleDeleteCreateProforma,
  handleGetCreateInvoice,
  handleGetCreateInvoiceOne,
  handleGetCreateProforma,
...
function Proforma(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        ...
        const response = await handleGetCreateProforma();
    ...
    <tr key={row.label}>
      ...
      <td justifycontent="right" align="right">
        <Menu shadow="sm" size="xs">
          ...
          <Menu.Dropdown>
            <Menu.Item
              onClick={() => navigate("/invoice_proforma/" + row.value)}
              icon={<Eye size={14} />}
            >
              View
            </Menu.Item>
  ...
  const handleConfirmDelete = async (e) => {
    const response = await handleDeleteCreateProforma(e);
  ...
  return (
    <div>
      ...
      <Skeleton radius="md" visible={variables.skeletonLoading}>
        <BreadCrumb Text="All Proforma Invoice" />
      </Skeleton>
